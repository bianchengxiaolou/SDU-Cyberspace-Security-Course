# -*- coding: utf-8 -*-
"""
Created on Sat Jul 30 12:43:41 2022
...
if __name__ == '__main__':
    str= input("消息为：")
    start=time.time()
    result= Hash_reduced_SM3(str)
    end=time.time()
    print("SM3哈希值为：",result)
    print("所用时间为",end-start,"s")
    
    
    
    n=int(input("碰撞位数为："))      #naïve birthday attack
    for i in range(2**n):
      x_0=decimalToHex(random.randint(0,2**512))
      y_0=decimalToHex(random.randint(0,2**512))
      x_1=Hash_reduced_SM3(x_0)
      y_1=Hash_reduced_SM3(y_0)
      n_hex=n//8
      if(x_1[:n_hex]==y_1[:n_hex]):
          print("成功找到一对碰撞")
          print(x_0,y_0)
          break
      else:
